...
segy_file = data_folder / "sfs_sh_shots_6-55.segy"
...
# SH beam survey
select_horizontal_component = [i for i, channel in enumerate(sfs.channel_numbers) if 1 <= channel <= 48]
sfs.select_traces(select_horizontal_component)
segy_outfile = segy_file.with_stem(segy_file.stem + "_SH10HZ_SORTED")
...
#other selection options, or cobined selection options


#select_horizontal_shots =     [i for i, ffid in enumerate(sfs.ffid) if 1005 <= ffid <= 1060] #logbook qc needed
#select_vertical_shots =       [i for i, ffid in enumerate(sfs.ffid) if 1061 <= ffid <= 1087] #logbook qc needed
#select_refraction_shots =     [i for i, ffid in enumerate(sfs.ffid) if 1090 <= ffid <= 1091] #logbook qc needed

# combine the selections
#sh_survey_selection = list(set(select_horizontal_component) & set(select_horizontal_shots))
#p_survey_selection = list(set(select_vertical_component) & set(select_vertical_shots))
#refraction_survey_selection = list(set(select_vertical_component) & set(select_refraction_shots))